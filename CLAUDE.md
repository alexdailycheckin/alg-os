...
- **Build 2 (canonical execution skills):** v0.2 complete. Four canonical skills are wired: artefact, teardown, proposal, nurture. Handoff was demoted from canonical (situational, not universal) and operators with delivery teams write their own in `/workspace/execution/handoff/`. The artefact skill ships edition-1 production capability in v0.3 (the spec is decoration if the operator cannot also produce edition 1 from their data).
- **Build 3 (build my agents):** not started. Reframed from "tool-agnostic recommendation engine" to "meta-agent that builds the operator's running agent stack." The meta-agent ingests foundation files plus the artefact spec and produces working agent definitions, MCP connections, and scheduled triggers so the operator's motion runs autonomously rather than requiring manual skill invocation.
